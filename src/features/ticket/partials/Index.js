...
export const getTicketsMethod = (setLoading, setTickets, handlePagination) => {
  setLoading(true);
  getTickets(setLoading).then(response => {
    setLoading(false);
    handlePagination(response.data.tickets.ticket, true);
  });
};

export const getDepartmenListMethod = (setLoading, setDepartemanList) => {
  getDepartmenList(setLoading).then(response => {
