...
import {showNotification} from "./FactorsTableComponent.js"
...
function FactorsTableComponent({t, factors}) {
    ...
    return (
        <>
            <StyledTable>
                <StyledTableHeadTr>
                    <StyledTableCell align="center" width={100}>{t('translation:id')}</StyledTableCell>
                    <StyledTableCell align="center" width={100}>{t('tickets:ticketNum')}</StyledTableCell>
                    <StyledTableCell align="center" width={100}>{t('tickets:price')}</StyledTableCell>
                    <StyledTableCell align="center" width={100}>{t('tickets:timePay')}</StyledTableCell>
                    <StyledTableCell align="center" width={100}>{t('translation:status')}</StyledTableCell>
                    <StyledTableCell align="center" width={100}>{t('translation:date')}</StyledTableCell>
                </StyledTableHeadTr>
                <StyledTableBody>
                    {factors.length > 0
                        ? (factors.map((factor, index) =>
                            (<React.Fragment key={index}>
                                <StyledTableBodyRow>
                                    ...
                                    <StyledTableCell align="center" width={100}>
                                        {lang === "en" ? convertDashToSlashInDate(factor.duedate):
                                            toShamsiDate(convertDashToSlashInDate((factor.created_at)))
                                        }
                                    </StyledTableCell>
                                    <StyledTableCell align="center" width={100}>
                                            {factor.status === "Unpaid" ? (
                                                <form method="post" action="https://drupalhelp.ir/payInvoice.php">
                                                    ...
                                                    <StyledStatusBtn status={factor.status === "Unpaid"} type="submit">{t('tickets:pay')}</StyledStatusBtn>
                                                </form>) :t('tickets:paid')}
                                    </StyledTableCell>
                                    <StyledTableCell align="center" width={100}>
                                        {lang === "en" ? formatDate(factor.created_at):
                                       toShamsiDate(formatDate((factor.created_at)))
                                        }
                                    </StyledTableCell>
                                </StyledTableBodyRow>
                            </React.Fragment>)
