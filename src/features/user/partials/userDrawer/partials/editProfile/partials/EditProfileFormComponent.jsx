...
import {
    StyledFlexColumn, StyledFlexItemInside, StyledFormControl, StyledHeightInput,
    StyledRoleMargin, StyledRolesBlock
...
function EditProfileFormComponent({ t, user, setUser, defaultRoles, setDefaultRoles }) {
    ...
    const handleChange = (e, field) => {
        handleChangeMethod(t, e, user, field, setUser, setErrors, userNameList, mailList)
    }
    ...
    const edit = () => {
        editUserMethod(user.uid[0].value, user, appContext, t, getEditedUser, errors)
    }

    useEffect(() => {
        getUsers(appContext, setUserNameList, setMailList)
        getRolesMethod(setEnRoles, setFaRoles, appContext)
    }, [])

    return (<StyledEditProfileBody>
        <StyledFlexColumn>
            ...
            <StyledFlexItemInside>
                ...
                <StyledFormControl component="fieldset">
                    ...
                    <StyledRadioButton>
                        <RadioGroup className="status" aria-label="status" name="status" value={user.status[0].value}
