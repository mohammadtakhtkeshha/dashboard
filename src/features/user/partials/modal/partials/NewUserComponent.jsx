...
function NewUserComponent({
  ...
  const saveUser = () => {
    saveUserMethod(user, setLoading, getRegisteredUser, errors, closeForm);
  };

  const editUser = () => {
    editUserMethod(id, user, setLoading, getEditedUser, errors);
  };
  ...
  const handleChange = (e, field) => {
    handleChangeMethod(e, user, id, field, setUser, gottenMail, gottenName, userNameList, setErrors, userMailList, confirmPass);
  };
          ...
          <StyledFlexColumn>
            ...
            <StyledFlexItemInside>
              ...
              <StyledHeightInput>
                ...
                {errors.pass ? (
                  <>
                    {errors.pass.required ? <StyledTypographyError>{errors.pass.required}</StyledTypographyError> : ''}
                    {errors.pass.valid ? <StyledTypographyError>{errors.pass.valid}</StyledTypographyError> : ''}
                  </>
                ) : (
                  ''
                )}
              ...
              <StyledHeightInput>
                ...
                />
                {errors.confirmPass ? (
                  <> {errors.confirmPass.harmony ? <StyledTypographyError>{errors.confirmPass.harmony}</StyledTypographyError> : ''}</>
                ) : (
                  ''
                )}
              </StyledHeightInput>
              <StyledFormControl component="fieldset">
                ...
                <StyledRadioButton>
                  <RadioGroup className="status" aria-label="status" name="status" value={user.status[0].value} onChange={handleStatusChange}>
