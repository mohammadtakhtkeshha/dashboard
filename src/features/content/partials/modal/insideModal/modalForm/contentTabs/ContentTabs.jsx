...
function NewContentTabsComponent({ t, newsCategory, states, value, setValue }) {
  ...
  useEffect(() => {
    setTitleValidationMethod(contentContext.setErrors,contentContext.content.title);
  }, [contentContext.content.title,contentContext.setErrors]);//Once

  useEffect(() => {
    setFieldImageContentTypeGallaryErrorMethod(contentContext.contentType,contentContext.content.field_image,contentContext.setErrors);
  }, [contentContext.contentType,contentContext.content.field_image,contentContext.setErrors]);//Once

  useEffect(() => {
    changeErrorsWhenFillFieldImageMethod(contentContext.contentType,contentContext.content.field_image,contentContext.setErrors);
  }, [contentContext.contentType,contentContext.content.field_image,contentContext.setErrors]);//Once
  ...
  return (
    <>
      <StyledTabs value={value} onChange={handleTabChange}>
        ...
        <StyledTabFiles contentype={contentContext.contentType} label={t('translation:files')} {...a11yProps(2)} />
        <Tab label={t('contents:publishSetting')} {...a11yProps(3)} />
        <StyledTabSeo contentype={contentContext.contentType} label={t('contents:seo')} {...a11yProps(4)} />
        <StyledTabPortal contentype={contentContext.contentType} label={t('contents:portal')} {...a11yProps(5)} />
      </StyledTabs>
      <StyledTabPanels>
        <TabPanel value={value} index={0}>
          <Box>
            <TitleAndImgComponent contentype={contentContext.contentType} newsCategory={newsCategory} />
          </Box>
        </TabPanel>
      ...
      <NewContentFooterComponent contentype={contentContext.contentType} value={value} setValue={setValue} />
    </>
  );
}
