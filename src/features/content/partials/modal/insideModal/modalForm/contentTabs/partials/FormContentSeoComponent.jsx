...
function FormContentSeoComponent({ t, contentype }) {
  ...
  useEffect(() => {
    seoChangedMethod(contentsContext.setContent, title, description, keywords);
  }, [contentsContext.setContent,title,description,keywords]);

  return (
    <StyledRowBox contentype={contentype}>
      <StyledRow>
        <StyledCol className="seo-path">
          <StyledMt1>
            <StyledCheckboxComponent
              value={contentsContext.content.field_alias_status[0].value}
          ...
          <StyledInput
            ...
            className="path"
            onChange={e => handleChangePathMethod(e, contentsContext)}
          />
          {contentsContext.errors.path && (
            <StyledTypographyError align={lang === 'en' ? 'left' : 'right'}>{contentsContext.errors.path}</StyledTypographyError>
          )}
        </StyledCol>
      </StyledRow>
      <StyledRow>
