...
import { StyledTableImg, StyledCheckboxImgInTable, StyledTableCell } from 'assets/js/library/components/table';
...
function UserDashboardComponent({ t }) {
  ...
  useEffect(() => {
    getTenNumberOfUsers(setLoading).then(response => {
      let users = response.data;
      setUsers([...users]);
    });
  }, [setUsers, setLoading]); //Once
          ...
          <StyledPaper lang={lang}>
            ...
            <StyledDashboardTable>
              <StyledTableHeadRow lang={lang}>
                ...
                <StyledTableCell width="50" align={leftRightAlign}>
                  {t('users:username')}
                </StyledTableCell>
                <StyledTableCell width="45" align={leftRightAlign === 'left' ? 'right' : 'left'}>
                  {t('users:email')}
                </StyledTableCell>
                    ...
                    <StyledTableBodyRow key={index}>
                      <StyledTableCell width="8" align={leftRightAlign} minWidth={52}>
                        <StyledCheckboxImgInTable minWidth="90">
                          <StyledTableImg>
                            {user.picture ? <img src={user.picture} alt="user.name" /> : <img src={userImg} alt={userImg} />}
                          </StyledTableImg>
