    ...
    <StyledTableParent length={mostSeen.length}>
      <StyledTablePaper lang={lang}>
        ...
        <StyledMatamoTable>
          ...
          {chunks.length > 0 &&
            chunks[page].map((item, index) => (
              <StyledMatamoTableRow key={index}>
                <StyledTableCell width="20" align={leftRightAlign}>
                  <StyledFlexColumn>
                    <StyledFlexColumn>
                      <StyledDate>
                        <div> {item.serverTimePrettyFirstAction}</div>
                        <div>{item.serverDatePretty}</div>
                      </StyledDate>
                      {/*<StyledDate> {item.visitDurationPretty}</StyledDate>*/}
                    </StyledFlexColumn>
                    <StyledDate>
                      <div> IP: {item.visitIp}</div>
                    </StyledDate>
                    <StyledDate>
                      {item.referrerName}
                      <StyledIconMatamo src={`https://foroshgahsaz.ir/matomo/${item.referrerSearchEngineIcon}`} alt="" />
                    </StyledDate>
                  </StyledFlexColumn>
                </StyledTableCell>
                <StyledTableCell width="20" align={leftRightAlign}>
                ...
                <StyledTableCell width="80" align={leftRightAlign}>
                  ...
                  {item.actionDetails.map((action, index) => (
                    <div key={index}>
                      <span>
                        {' '}
                        <StyledImgAction src={actionSvg} alt="" />
                      </span>
                      <span>{action.pageTitle}</span>
                      <div>{action.url}</div>
                    </div>
                  ))}
                </StyledTableCell>
