...
function DevicesComponent({ t }) {
  ...
  useEffect(() => {
    getDevicesMethod(setLoading, setDevices);
  }, [setLoading, setDevices]);
              ...
              devices.map((device, index) => (
                <StyledMatamoTableRow key={index}>
                  <StyledMatamoTableCell align="right">
                    <StyledIconAndLabel>
                      {device.logo && <img src={`http://foroshgahsaz.ir/matomo/plugins/Morpheus/icons/dist/devices/${device.logo}`} alt="" />}
                      <span>{device.label}</span>
                    </StyledIconAndLabel>
                  </StyledMatamoTableCell>
                  <StyledMatamoTableCell align="right">{device.nb_visits === 0 ? '-' : device.nb_visits}</StyledMatamoTableCell>
                </StyledMatamoTableRow>
              ))}
            <StyledMatamoTabelFooter lang={lang}>
              <NavLink to="/report/devices">{t('translation:moreItems')}</NavLink>
            </StyledMatamoTabelFooter>
          </StyledMatamoTable>
        </StyledTablePaper>
      </StyledTableParent>
