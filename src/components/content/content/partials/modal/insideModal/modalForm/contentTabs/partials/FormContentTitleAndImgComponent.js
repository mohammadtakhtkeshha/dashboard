...
export const handleChangeNewsCategoryMethod = (e, isChecked, contentContext, setSelectedNewsCategory) => {
    const id = e.currentTarget.value;
    if (isChecked) {
        contentContext.setContent(prevState => {
            return {
...
export const handleChangeAutoPathMethod = (check, contentContext) => {
    if (check === false) {
        contentContext.setContent(prevState => {
            return {
