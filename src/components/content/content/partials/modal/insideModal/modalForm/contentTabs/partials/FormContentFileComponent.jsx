...
import UploadFile from "components/partials/UploadImgComponent.jsx";
import AppContext from "contexts/AppContext";
import {
    removeMultiImgMethod,
    uploadVideoMethod,
    uploadVoiceMethod,
    removeVideoMethod,
    removeVoiceMethod
...
import {
    styledGridSoundFile,
    styledGridVideoFile
} from "assets/js/content/partials/modal/insideModal/modalForm/contentTabs/contentTabs"
...
function FormContentFileComponent({t, contentype}) {
    ...
    const uploadVideo = (files) => {
        uploadVideoMethod(files, contentsContext, appContext);
    }

    const uploadVoice = (files) => {
        uploadVoiceMethod(files, contentsContext, appContext)
    }
    ...
    const uploadImg = (e, multiple) => {
        uploadImgMethod(e, multiple, contentsContext, appContext);
    }
    ...
    return (<Grid container>
        ...
        <StyledGridMultiImgBlock contentype={contentype} item xs={12} className="images">
            <UploadImgComponent type="image"
                                ...
                                multiple={true}/>
            <StyledImgsInputBlock
                showImgInputs={(contentsContext.content.field_images_gallery !== undefined && contentsContext.content.field_images_gallery.length) > 0 ? true : false}>
            </StyledImgsInputBlock>
        </StyledGridMultiImgBlock>
    </Grid>);
}
