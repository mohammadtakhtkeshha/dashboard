...
function checkPass(pass, type, id) {
    ...
    if (!regex.test(pass)) {
        valid = 'پسورد مورد نظر باید شامل اعداد حروف بزرگ و کوچک و علامت ها خاص باشد!'
    }
...
export const uploadImgMethod = (e, setUser, setImgAndUrl, appContext) => {
    ...
    userService.saveUserImage(e, appContext.handleError)
        .then((response) => {
...
export const saveUserMethod = (t, user, confirmPass, appContext, getRegisteredUser, errors,closeForm) => {
    if (isObjectEmpty(errors)) {
        appContext.setLoading(true)
        userService.registerUser(user, appContext.handleError).then((response) => {
...
export const getUserMethod = (appContext, id, setDefaultRoles, setUser, setGottenMail, setImgAndUrl, setGottenName) => {
    if (id) {
        appContext.setLoading(true)
        userService.getUser(id, appContext.handleError).then((response) => {
            appContext.setLoading(false)
...
export const getRolesMethod = (setEnRoles, setFaRoles) => {
